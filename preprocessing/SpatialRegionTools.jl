...
function makeVocab!(region::SpatialRegion, trjfile::String)
    ...
    println("*")
    println("Cell count at max_num_hotcells:$(max_num_hotcells)")
    println("*")
    println("$(last(topcellcount[end]))")
    println("*")
...
function createTrainVal(region::SpatialRegion,
    ...
    trjfile = "$datapath/$name_valtest.h5"
	println("Opening H5 file at $(trjfile)")
    h5open(trjfile, "r") do f
        trainsrc = open("$datapath/$name_valtest.src", "w")
        traintrg = open("$datapath/$name_valtest.trg", "w")
        trainmta = open("$datapath/$name_valtest.mta", "w")
        
        num = read(attributes(f)["num"])
	
        for i = 1:num
            trip = f["/trips/$i"] |> read
            #min_length <= size(trip, 2) <= max_length || continue # We dont to that
            ...
            i % 10_000 == 0 && println("Scaned $i trips...")
            #i >= 8_000 && break
        end
        close(trainsrc), close(traintrg), close(trainmta)
    end
    nothing
end
